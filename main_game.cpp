...
bool MainGame::init()
{
	...
	spr_bg1_ = Sprite::create("image/background.png");
	spr_bg1_->setAnchorPoint(ccp(0, 0));
	spr_bg1_->setPosition(ccp(0, 0));
	this->addChild(spr_bg1_);

	spr_bg2_ = Sprite::create("image/background.png");
...
void MainGame::init_hero_plane(int index)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		auto str_name = String::createWithFormat("image/heros%d_%d.png", index, i + 1);
	...
	//精灵
	hero_player_ = Sprite::create(String::createWithFormat("image/heros%d_1.png", index)->getCString());
	auto size = Director::getInstance()->getWinSize();
...
void MainGame::is_crash(float tm)
{
	...
	if (plane_support_ != NULL)
	{
		...
		{
			//添加支援物资的子弹
			this->schedule(SEL_SCHEDULE(&MainGame::add_support_bullet), 0.1f, 50, 0);//停止普通子弹
...
void MainGame::hero_death()
{
	...
	if (score_ > score_win)
	{
		...
		Total_ = score_ + score_win;
		UserDefault::getInstance()->setIntegerForKey("Total", Total_);
	}
	...
	for (int i = 1; i <= 4; ++i)
	{
		auto str_name = String::createWithFormat("image/b%d.png", i);
		animation->addSpriteFrameWithFile(str_name->getCString());
	}
