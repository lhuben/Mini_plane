...
void EenmyBase::initEnemy(int name_index /* = 1 */)
{
	...
	string name_str = String::createWithFormat("image/enemy%d.png", name_index)->getCString();
...
void EenmyBase::enemy_death()
{
	...
	for (int i = 1; i <= 4; ++i)
	{
		auto  enemy_name = String::createWithFormat("image/b%d.png", i);
		animation->addSpriteFrameWithFile(enemy_name->getCString());
	}
	animation->setDelayPerUnit(0.2f);
	animation->setLoops(1);
	this->runAction(Sequence::create(Animate::create(animation), RemoveSelf::create(), CallFunc::create(CC_CALLBACK_0(EenmyBase::remove_enemy, this)), NULL));//顺序执行创建 删除 和回调函数
}
